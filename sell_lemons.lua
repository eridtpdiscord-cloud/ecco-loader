...
local function toggleOverlay(enabled)
    ...
    -- ============================================================
    -- NOTIFICATION DROPDOWN SYSTEM (GRID MOVEMENT CONTAINER)
    -- ============================================================
    local notifDropdownContainer = Instance.new("Frame")
    notifDropdownContainer.Size = UDim2.new(0.9, 0, 0, 36)
    notifDropdownContainer.Position = UDim2.new(0.05, 0, 0.105, 0)
    notifDropdownContainer.BackgroundTransparency = 1
    notifDropdownContainer.ZIndex = 10
    notifDropdownContainer.Parent = bg

    local notifToggleBtn = Instance.new("TextButton")
    notifToggleBtn.Size = UDim2.new(0, 240, 0, 30)
    notifToggleBtn.Position = UDim2.new(0, 0, 0, 0)
    notifToggleBtn.BackgroundColor3 = Color3.fromRGB(24, 28, 42)
    notifToggleBtn.TextColor3 = Color3.fromRGB(0, 220, 255)
    notifToggleBtn.Text = "🔔 Cloud Broadcasts (3) ▼"
    notifToggleBtn.Font = Enum.Font.SourceSansBold
    notifToggleBtn.TextSize = 14
    notifToggleBtn.ZIndex = 11
    notifToggleBtn.Parent = notifDropdownContainer

    local notifBtnCorner = Instance.new("UICorner")
    notifBtnCorner.CornerRadius = UDim.new(0, 8)
    notifBtnCorner.Parent = notifToggleBtn

    local notifBtnStroke = Instance.new("UIStroke")
    notifBtnStroke.Color = Color3.fromRGB(0, 180, 255)
    notifBtnStroke.Thickness = 1.2
    notifBtnStroke.Transparency = 0.4
    notifBtnStroke.Parent = notifToggleBtn

    -- Dropdown Panel Frame
    local notifDropdown = Instance.new("Frame")
    notifDropdown.Size = UDim2.new(0, 480, 0, 0)
    notifDropdown.Position = UDim2.new(0, 0, 0, 36)
    notifDropdown.BackgroundColor3 = Color3.fromRGB(15, 18, 28)
    notifDropdown.BackgroundTransparency = 0.05
    notifDropdown.ClipsDescendants = true
    notifDropdown.Visible = false
    notifDropdown.ZIndex = 12
    notifDropdown.Parent = notifDropdownContainer

    local notifDropCorner = Instance.new("UICorner")
    notifDropCorner.CornerRadius = UDim.new(0, 10)
    notifDropCorner.Parent = notifDropdown

    local notifDropStroke = Instance.new("UIStroke")
    notifDropStroke.Color = Color3.fromRGB(0, 200, 255)
    notifDropStroke.Thickness = 1.5
    notifDropStroke.Transparency = 0.2
    notifDropStroke.Parent = notifDropdown

    -- Dropdown Header inside
    local notifDropHeader = Instance.new("Frame")
    notifDropHeader.Size = UDim2.new(1, 0, 0, 32)
    notifDropHeader.BackgroundColor3 = Color3.fromRGB(20, 24, 38)
    notifDropHeader.BorderSizePixel = 0
    notifDropHeader.ZIndex = 13
    notifDropHeader.Parent = notifDropdown

    local notifDropTitle = Instance.new("TextLabel")
    notifDropTitle.Size = UDim2.new(0.65, 0, 1, 0)
    notifDropTitle.Position = UDim2.new(0, 12, 0, 0)
    notifDropTitle.Text = "📢 ECCO CLOUD BROADCASTS"
    notifDropTitle.TextColor3 = Color3.fromRGB(255, 215, 0)
    notifDropTitle.Font = Enum.Font.SourceSansBold
    notifDropTitle.TextSize = 13
    notifDropTitle.TextXAlignment = Enum.TextXAlignment.Left
    notifDropTitle.BackgroundTransparency = 1
    notifDropTitle.ZIndex = 14
    notifDropTitle.Parent = notifDropHeader

    local notifRefreshBtn = Instance.new("TextButton")
    notifRefreshBtn.Size = UDim2.new(0, 90, 0, 24)
    notifRefreshBtn.Position = UDim2.new(1, -100, 0, 4)
    notifRefreshBtn.BackgroundColor3 = Color3.fromRGB(30, 36, 56)
    notifRefreshBtn.TextColor3 = Color3.fromRGB(200, 220, 255)
    notifRefreshBtn.Text = "🔄 Refresh"
    notifRefreshBtn.Font = Enum.Font.SourceSansBold
    notifRefreshBtn.TextSize = 12
    notifRefreshBtn.ZIndex = 14
    notifRefreshBtn.Parent = notifDropHeader

    local refCorner = Instance.new("UICorner")
    refCorner.CornerRadius = UDim.new(0, 6)
    refCorner.Parent = notifRefreshBtn

    local notifScroll = Instance.new("ScrollingFrame")
    notifScroll.Size = UDim2.new(1, -16, 1, -40)
    notifScroll.Position = UDim2.new(0, 8, 0, 36)
    notifScroll.BackgroundTransparency = 1
    notifScroll.ScrollBarThickness = 4
    notifScroll.ScrollBarImageColor3 = Color3.fromRGB(0, 200, 255)
    notifScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y
    notifScroll.ZIndex = 13
    notifScroll.Parent = notifDropdown
    ...
    notifListLayout.Parent = notifScroll

    local function populateNotifs(notifData)
        for _, child in ipairs(notifScroll:GetChildren()) do
            if child:IsA("Frame") then child:Destroy() end
        end
        local list = notifData or {
            {title = "Ecco Hub V3 Online", content = "All 320+ place scripts synchronized with cloud config and keyless bypass.", badge = "UPDATE", date = "2026-09-15"},
            {title = "New Game Releases", content = "Storage Hunters & Sell Lemons automation profiles active.", badge = "NEW", date = "2026-09-14"},
            {title = "Discord Announcements", content = "Join discord.gg/ecc00 for instant patch updates and script drops.", badge = "ANNOUNCEMENT", date = "2026-09-12"}
        }
        notifToggleBtn.Text = "🔔 Cloud Broadcasts (" .. tostring(#list) .. ") ▼"
        for _, item in ipairs(list) do
            local itemFrame = Instance.new("Frame")
            itemFrame.Size = UDim2.new(1, -6, 0, 68)
            itemFrame.BackgroundColor3 = Color3.fromRGB(22, 26, 40)
            itemFrame.BackgroundTransparency = 0.2
            itemFrame.BorderSizePixel = 0
            itemFrame.ZIndex = 14
            itemFrame.Parent = notifScroll

            local itCorner = Instance.new("UICorner")
            itCorner.CornerRadius = UDim.new(0, 8)
            itCorner.Parent = itemFrame

            local itStroke = Instance.new("UIStroke")
            itStroke.Color = Color3.fromRGB(50, 60, 90)
            itStroke.Thickness = 1
            itStroke.Parent = itemFrame

            local badgeColor = Color3.fromRGB(0, 200, 255)
            if item.badge == "ALERT" or item.badge == "HOTFIX" then
                badgeColor = Color3.fromRGB(255, 70, 70)
            elseif item.badge == "NEW" or item.badge == "RELEASE" then
                badgeColor = Color3.fromRGB(50, 255, 150)
            elseif item.badge == "ANNOUNCEMENT" then
                badgeColor = Color3.fromRGB(255, 200, 50)
            end

            local badgeLbl = Instance.new("TextLabel")
            badgeLbl.Size = UDim2.new(0, 80, 0, 18)
            badgeLbl.Position = UDim2.new(0, 8, 0, 6)
            badgeLbl.BackgroundColor3 = badgeColor
            badgeLbl.BackgroundTransparency = 0.8
            badgeLbl.TextColor3 = badgeColor
            badgeLbl.Text = "[" .. tostring(item.badge or "NOTICE") .. "]"
            badgeLbl.Font = Enum.Font.SourceSansBold
            badgeLbl.TextSize = 11
            badgeLbl.ZIndex = 15
            badgeLbl.Parent = itemFrame

            local bCorner = Instance.new("UICorner")
            bCorner.CornerRadius = UDim.new(0, 4)
            bCorner.Parent = badgeLbl

            local itemTitle = Instance.new("TextLabel")
            itemTitle.Size = UDim2.new(1, -100, 0, 18)
            itemTitle.Position = UDim2.new(0, 94, 0, 6)
            itemTitle.Text = tostring(item.title or "Broadcast")
            ...
            local itemBody = Instance.new("TextLabel")
            itemBody.Size = UDim2.new(1, -16, 0, 36)
            itemBody.Position = UDim2.new(0, 8, 0, 26)
            itemBody.Text = tostring(item.content or "")
            itemBody.TextColor3 = Color3.fromRGB(180, 190, 210)
            itemBody.Font = Enum.Font.SourceSans
            itemBody.TextSize = 12
            itemBody.TextWrapped = true
            itemBody.TextXAlignment = Enum.TextXAlignment.Left
            itemBody.TextYAlignment = Enum.TextYAlignment.Top
            itemBody.BackgroundTransparency = 1
            itemBody.ZIndex = 15
            itemBody.Parent = itemFrame
        end
    end

    populateNotifs()

    local isNotifOpen = false
    notifToggleBtn.MouseButton1Click:Connect(function()
        isNotifOpen = not isNotifOpen
        if isNotifOpen then
            notifDropdown.Visible = true
            TweenService:Create(notifDropdown, TweenInfo.new(0.25, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
                Size = UDim2.new(0, 480, 0, 240)
            }):Play()
            notifToggleBtn.Text = string.gsub(notifToggleBtn.Text, "▼", "▲")
        else
            local tw = TweenService:Create(notifDropdown, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {
                Size = UDim2.new(0, 480, 0, 0)
            })
            tw:Play()
            tw.Completed:Connect(function()
                if not isNotifOpen then notifDropdown.Visible = false end
            end)
            notifToggleBtn.Text = string.gsub(notifToggleBtn.Text, "▲", "▼")
        end
    end)

    notifRefreshBtn.MouseButton1Click:Connect(function()
        notifRefreshBtn.Text = "Fetching..."
        task.spawn(function()
            ...
            if ok and raw then
                ...
                if s and parsed and parsed.notifications then
                    populateNotifs(parsed.notifications)
                end
            end
            notifRefreshBtn.Text = "🔄 Refresh"
        end)
